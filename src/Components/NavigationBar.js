    ...
    <header className="w-full px-3">
        <div className="flex flex-row items-center basis-10/12 w-full">
            <nav className="flex flex-col sm:flex-row items-center justify-center my-3 px-1 w-full">
                ...
                <ul className={` ${isSideMenuTrigger ? '':'hidden'}  md:flex justify-center basis-8/12 flex-col md:flex-row absolute md:relative right-0 top-0 bottom-0 w-60 bg-white md:bg-transparent`}>
                    ...
                    <li className="px-2 mx-2 color-bg-white rounded-full flex items-center md:justify-center my-4 md:my-0"><Link to="/" className='body-font-5 font-bold'>HOME</Link></li>
                    <li className="px-2 mx-2 color-bg-white rounded-full relative " onMouseEnter={()=>setIsNewsHover(true)} onMouseLeave={()=>setIsNewsHover(false)}>
                ...
                <form className="flex flex-row basis-9/12 md:basis-4/12 justify-end w-full px-5 md:px-0">
                    ...
                    <button id='search' aria-label="Search Button" type="button"><img src="/assets/icons/search-icon.svg" alt="" className='w-8 bg-white h-full p-1 rounded-r-lg'/></button>
                </form>
            </nav>
