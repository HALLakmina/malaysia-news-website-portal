...
const NewsCard = ({ category, data}) => {
  ...
  return (
      <div className="rounded-lg p-3 color-bg-white flex flex-col relative my-2 mx-2 ">
          ...
          <Link to={`/news/${category}/${data._id}`}> <img src={`${process.env.REACT_APP_API_URL}/${data.image.storageName}`} alt="" className="size-full rounded-lg object-contain overflow-hidden"/></Link>
          </div>
